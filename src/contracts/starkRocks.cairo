...
from starkware.cairo.common.uint256 import Uint256,uint256_le,uint256_mul_div_mod
...
// STRUCTS
struct RoyalityInfo {
    receiver: felt, // Royality receiver
    royality_fee: Uint256, // Royality amount
}
...
// Royality Info
@storage_var
func Royality_Info() -> (info: RoyalityInfo) {
}
...
@view
func mintPrice{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() -> (price: Uint256) {
    return Mint_Price.read();
}
...
@view
func merkleRoot{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() -> (root: felt) {
    return Merkle_Root.read();
}
...
@external
func setRoyaltyInfo{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    _receipt: felt, _royality_fee: Uint256
) {
    alloc_locals;
    Ownable.assert_only_owner();
    let _RoyalityInfo = RoyalityInfo(
            receiver=_receipt,
            royality_fee=_royality_fee
        );
    Royality_Info.write(_RoyalityInfo);
    return ();
}

@external
func setMintPrice{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    _price: Uint256
) {
    Ownable.assert_only_owner();
...
@external
func setMerkleRoot{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(root: felt
) {
    Ownable.assert_only_owner();
    Merkle_Root.write(root);
    return ();
...
// INTERNAL
func _teamMint{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    index:felt, wallet_addresses_len:felt, wallet_addresses: felt*
) {
    if (index == wallet_addresses_len) {
        return ();
    }
    Total_Supply.write(index+1);
    _mint(wallet_addresses[index], index+1);
    _teamMint(index = index + 1,
    wallet_addresses_len = wallet_addresses_len,
